#A função "separar_data" pega a data de nascimento digitada e divide ela separada mente nas variaveis d,m e a.
...
#Em seguinda a função "signo" seleciona o signo referente a data digitada pelas variaveis d(dia) e m(mês).
def signo(dia, mes):
    if mes == 3:
...
#A função "remover_acentos" retorna a string sem acentuação ou caracteres especiais.
...
#A função "horoscopo" retorna o horóscopo do signo desejado,após a decodificação na página do conteudo.
...
#Na função "main" ocorre o processo de entrada de dados, processamento e por fim a saída de dados imprimidos na tela.
def main():
#Entrada de dados
    nascimento = int(input("Digite sua data de nascimento em DDMMAAAA: "))
